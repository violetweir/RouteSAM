...
def main():
 ...
 rows=read(root/'data/train_manifest.jsonl');assert len(rows)==588 and sum(x['is_gt'] for x in rows)==8
 ...
 try:
  for epoch in range(1,817):
   ...
   assert seen==[rows[i]['target_id'] for i in order] and len(set(seen))==588 and gt_count==8
   row=dict(epoch=epoch,iteration=total_steps,steps_in_epoch=len(losses),loss=float(np.mean(losses)),lr=lr,gt_count=gt_count,pseudo_count=588-gt_count,zero_gt_batches=zero_gt_batches,unique_images=len(set(seen)),order_sha256=hashlib.sha256('\n'.join(seen).encode()).hexdigest(),elapsed_seconds=time.time()-start)
