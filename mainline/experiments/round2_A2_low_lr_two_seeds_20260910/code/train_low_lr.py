...
def main():
    D, R = d.D, d.R
    arm = d.args.arm
    assert arm in ['seed2026','seed2027']
    seed=int(arm[4:])
    expected_count = 511
    total_steps = expected_count*10
    D.mkdir(parents=True, exist_ok=False)
    d.save(D/'status.json', dict(stage='initializing', arm='A2_low_lr_'+arm, gpu=d.args.gpu))
    ...
    for epoch in range(1,11):
        ...
        for j,batch in enumerate(loader):
            ...
            if j==0 or step%20==0:
                status=dict(stage='training',arm='A2_low_lr_'+arm,epoch=epoch,epochs=10,epoch_step=j+1,epoch_size=expected_count,
    ...
    other='seed2027' if seed==2026 else 'seed2026'
    d.save(D/'status.json',dict(stage='waiting_for_other_seed',best_epoch=best_epoch))
    model.cpu();trainer.optimizer.state.clear();d.torch.cuda.empty_cache()
    while not (R/'runs'/other/'WEIGHTS_FROZEN.json').exists():
        if (R/'runs'/other/'FAILED.txt').exists():raise RuntimeError('Other seed failed; hold test')
        time.sleep(30)
    model.cuda()
    ...
    d.save(D/'results.json',dict(arm='A2_low_lr_'+arm,best_epoch=best_epoch,validation=best,test=result,test_empty=empty_result,training_pool_size=expected_count,
    ...
    report='\n'.join([f'# {arm}新设置训练结果','',f'Best epoch: {best_epoch}; Val Dice: {best["dice"]:.9f}',
        ...
        '原图直接1008输入；256逐图宏平均指标；本次两个种子的权重按有文本validation冻结后才执行test。空文本为预先约定的补充消融。',''])
