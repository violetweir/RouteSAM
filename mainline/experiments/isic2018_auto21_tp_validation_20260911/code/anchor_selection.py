...
R=Path('/Data_8TB/lht/PseudoVideo-SAM3-X3-B7/new_project/experiments/isic2018_auto21_tp_validation_20260911/selection')
...
assert g.shape==(2075,1024) and sample.shape==(2075,64,1024)
save(R/'train_images_only.json',rows)
save(R/'input_provenance.json',dict(protocol_sha256=sha(protocol),features_sha256=sha(R.parent/'selection_features.npz'),train_image_hashes=dict(zip([r['id'] for r in rows],hashes)),eligible_count=len(eligible),ordering_checked=True))
...
assign=km.predict(sample.reshape(-1,1024)).reshape(2075,64)
h=np.stack([np.bincount(x,minlength=64) for x in assign]).astype(np.float32)/64
idf=np.log((2076)/(1+(h>0).sum(0)))+1
...
def greedy(s):
    chosen=[];best=np.zeros(2075,dtype=np.float32);trace=[]
    for k in range(64):
        gains=np.maximum(s-best[:,None],0).mean(0)
        allowed=np.zeros(2075,dtype=bool);allowed[eligible]=True;allowed[chosen]=False
...
for name,s in [('global_facility',sg),('global_local_facility',sm)]:
    ...
    stops[name]=dict(k=stop,reached_within_64=stop is not None,threshold=.01,normalizer=distortion)
rng=np.random.default_rng(2026)
random_sets=[rng.choice(eligible,64,replace=False).tolist() for _ in range(100)]
...
def metric(ix):
    ...
    for name,s in [('global',sg),('local',sl),('combined',sm)]:
        nearest=s[:,ix].max(1);non=np.ones(2075,dtype=bool);non[ix]=False
        ...
        out[name]=dict(mean_coverage=float(nearest.mean()),unselected_mean_coverage=float(nearest[non].mean()),p10_coverage=float(np.quantile(nearest[non],.1)),pairwise_similarity=float(v.mean()) if len(v) else None,max_assignment_share=float(counts.max()/2075),assignment_counts=counts.tolist())
    return out
metrics={'existing21':metric(old)};curves={};random_summary={}
for name,ix in sets.items():
    metrics[name]=metric(ix[:21]);curves[name]={str(k):metric(ix[:k]) for k in policy['budget_curve']}
...
save(R/'coverage_metrics.json',dict(budget21=metrics,budget_curves=curves,random_summary=random_summary,stopping=stops,warning='Optimized feature coverage is not independent validation and is not segmentation Dice. No propagation performed.'))
...
audits={};gallery=[]
for name,ix in [('existing21',old)]+[(name,ix[:21]) for name,ix in sets.items()]:
...
page='''<!doctype html><html lang="zh-CN"><meta charset="utf-8"><title>自动参考图选择：21张预算</title><style>body{background:#111923;color:#edf3fa;font:16px system-ui;margin:25px}section{margin:30px 0;border-top:1px solid #567}article{display:inline-block;vertical-align:top;width:46%;padding:1%;margin:1%;background:#202b38;box-sizing:border-box}h3{font-size:14px;overflow-wrap:anywhere}.pair{display:flex;align-items:flex-start;gap:5px}.pair img{width:49%;height:auto}p{color:#b7c8dc}@media(max-width:2075px){article{width:98%}}</style><h1>自动参考图选择：固定21张预算</h1><p>仅根据训练图像特征选图，名单冻结后才读取GT。GT用于本页展示，未用于选图。这是特征覆盖实验，不是分割性能结论。</p><p><a href="report.md">实验报告</a></p>'''+''.join(gallery)+'</html>'
...
rr=random_summary['21'];report+=f"|随机100组均值|{rr['global']['unselected_mean_coverage']['mean']:.6f}|{rr['local']['unselected_mean_coverage']['mean']:.6f}|{rr['combined']['unselected_mean_coverage']['mean']:.6f}|{rr['combined']['max_assignment_share']['mean']:.2%}|\n"
...
for name,x in stops.items():report+=f"- {name}: {x['k'] if x['k'] is not None else '64张内未达到停止条件'}。\n"
report+='\n## 冻结后GT形态审计\n\n'
...
save(R/'COMPLETE.json',dict(selection_frozen_sha256=freeze_sha,images_selected_per_method=21,train_only=True,old_experiments_unchanged=True))
status('complete')
