...
def main():
    ...
    arm = d.args.arm
    assert arm in ['A1','A2']
    expected_count = {'A1':604,'A2':511}[arm]
    total_steps = expected_count*10
    D.mkdir(parents=True, exist_ok=False)
    d.save(D/'status.json', dict(stage='initializing', arm=arm+'_v2', gpu=d.args.gpu))
    ...
    d.save(D/'initialization.json', dict(adapter_hash=initial_hash, frozen_hash=frozen_hash, matches_reference_initialization=True))
    ...
    for epoch in range(1,11):
        ...
        for j,batch in enumerate(loader):
            ...
            if j==0 or step%20==0:
                status=dict(stage='training',arm=arm+'_v2',epoch=epoch,epochs=10,epoch_step=j+1,epoch_size=expected_count,
    ...
    other = 'A2' if arm=='A1' else 'A1'
    d.save(D/'status.json',dict(stage='waiting_for_other_arm',best_epoch=best_epoch))
    model.cpu();trainer.optimizer.state.clear();d.torch.cuda.empty_cache()
    while not (R/'runs'/other/'WEIGHTS_FROZEN.json').exists():
        if (R/'runs'/other/'FAILED.txt').exists():raise RuntimeError('Other arm failed; test held')
        time.sleep(30)
    model.cuda()
    ...
    d.save(D/'results.json',dict(arm=arm+'_v2',best_epoch=best_epoch,validation=best,test=result,test_empty=empty_result,training_pool_size=expected_count,
    ...
    report='\n'.join([f'# {arm}新设置训练结果','',f'Best epoch: {best_epoch}; Val Dice: {best["dice"]:.9f}',
        ...
        '原图直接1008输入；256逐图宏平均指标；两组权重均按有文本validation冻结后才执行test。空文本为预先约定的补充消融。',''])
