"""User-requested, fixed A1 epoch-5 direct test audit; no training mutation."""
import json
import shutil
import traceback
...
def main():
    dest = d.R / 'audits/A1_epoch5_original1008_empty_test'
    dest.mkdir(parents=True, exist_ok=False)
    d.D = dest
    source = d.R / 'runs/A1/epoch5.pt'
    frozen = dest / 'epoch5.pt'
    shutil.copy2(source, frozen)
    digest = d.sha(source)
    assert d.sha(frozen) == digest
    val = json.loads((d.R / 'runs/A1/validation_epoch5/summary.json').read_text())
    d.save(dest / 'AUDIT_FROZEN.json', dict(
        arm='A1', epoch=5, source=str(source), checkpoint_sha256=digest,
        validation_effective256_reference=val, reason='User requested original1008 empty-text test for the same A1 epoch5 checkpoint',
    ...
    d.save(dest / 'status.json', dict(stage='test', epoch=5))
    result = d.evaluate(model, 'test', 'test')
    assert result['count'] == 100
    assert d.sha(source) == digest and d.sha(frozen) == digest
    d.save(dest / 'results.json', dict(epoch=5, checkpoint_sha256=digest, validation_effective256_reference=val, test=result, diagnostic=True))
    report = '\n'.join([
        ...
        '|---|---:|---:|---:|---:|',
        f"| A1 epoch 5 | {val['dice']:.9f} | {result['dice']:.9f} | {result['iou']:.9f} | {result['count']} |", '',
