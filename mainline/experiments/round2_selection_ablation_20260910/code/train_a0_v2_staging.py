"""A0 restart: original RGB to 1008, cosine per update, gradient norm clip 1."""
...
def main():
    ...
    d.save(D/'status.json', dict(stage='initializing', arm='A0_v2', gpu=d.args.gpu))
    d.torch.set_num_threads(4)
    trainer = d.t.SAM3TrainerNative(str(R/'A0.yaml'))
    model = trainer.model
    ...
    d.save(D/'initialization.json', dict(adapter_hash=initial_hash, frozen_hash=frozen_hash, matches_old_A0=True))
    ds = d.t.COCOSegmentDataset(R/'data/A0', 'train')
    records = json.loads((R/'data/A0/records.json').read_text())
    assert len(ds)==len(records)==436 and sum(x['is_gt'] for x in records)==8
    scheduler = d.torch.optim.lr_scheduler.CosineAnnealingLR(trainer.optimizer, T_max=4360, eta_min=5e-7)
    if d.args.smoke:
        sample = ds[0]
        ...
        return
    assert json.loads((R/'smoke/A0/smoke.json').read_text())['passed']
    ...
    for epoch in range(1,11):
        order=d.np.random.default_rng(2026+epoch).permutation(len(ds)).tolist()
        ...
        for j,batch in enumerate(loader):
            ...
            expected_lr=5e-7+(5e-5-5e-7)*(1+math.cos(math.pi*(step-1)/4360))/2
            assert math.isclose(info['learning_rate'],expected_lr,rel_tol=1e-9)
            d.append(D/'updates.jsonl',dict(epoch=epoch,step=step,**info))
            if j==0 or step%20==0:
                status=dict(stage='training',arm='A0_v2',epoch=epoch,epochs=10,epoch_step=j+1,epoch_size=436,
        ...
        assert len(updates)==436 and len(set(order))==436
    ...
    d.save(D/'results.json',dict(arm='A0_v2',best_epoch=best_epoch,validation=best,test=result,training_pool_size=436))
