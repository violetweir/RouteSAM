...
def assert_text_only(batch):
 assert list(batch.find_text_batch)==['colon polyp']
...
class DirectDataset(Dataset):
 ...
 def __getitem__(self,i):
  r=self.rows[i]
  image=PIL.open(r['file_name']).convert('RGB').resize((256,256),PIL.Resampling.BILINEAR).resize((1008,1008),PIL.Resampling.BILINEAR)
  ...
  query=t.FindQueryLoaded(query_text='colon polyp',image_id=0,object_ids_output=[0],is_exhaustive=True,query_processing_order=0,
