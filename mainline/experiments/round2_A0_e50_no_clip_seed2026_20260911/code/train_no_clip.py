"""A0 seed2026 50epochs: original1008, restored LR5e-5 cosine, NO gradient clipping."""
...
def update(trainer, batch, scheduler):
    ...
    loss_dict = trainer.loss_wrapper(out, targets)
    loss = loss_dict[d.t.CORE_LOSS_KEY]
    components = {k:float(v.detach()) if d.torch.is_tensor(v) else float(v) for k,v in loss_dict.items() if (not d.torch.is_tensor(v) or v.numel()==1)}
    ...
    norm = d.torch.linalg.vector_norm(d.torch.stack([d.torch.linalg.vector_norm(p.grad.detach().float()) for p in params]))
    assert bool(d.torch.isfinite(norm)), 'nonfinite gradient; stopped without clipping'
    after = norm
    lr = trainer.optimizer.param_groups[0]['lr']
    trainer.optimizer.step()
    scheduler.step()
    d.torch.clear_autocast_cache()
    return dict(loss_components=components, loss=float(loss.detach()), learning_rate=lr, next_learning_rate=trainer.optimizer.param_groups[0]['lr'],
                grad_norm_before=float(norm), grad_norm_after=float(after), clipped=False, gradient_clipping_enabled=False)

def monitor_training(model, tag):
    py=random.getstate();npstate=d.np.random.get_state()
    try:
        with d.torch.random.fork_rng(devices=[d.torch.cuda.current_device()]):
            return d.evaluate(model,'train_monitor',tag)
    finally:
        random.setstate(py);d.np.random.set_state(npstate)
...
def main():
    ...
    seed=int(arm[4:])
    expected_count = 436
    total_steps = expected_count*50
    D.mkdir(parents=True, exist_ok=False)
    d.save(D/'status.json', dict(stage='initializing', arm='A0_e50_no_clip_'+arm, gpu=d.args.gpu))
    ...
    old = json.loads((R/f'reference_{arm}.json').read_text())
    assert initial_hash == old['adapter_hash'] and frozen_hash == old['frozen_hash']
    d.save(D/'initialization.json', dict(adapter_hash=initial_hash, frozen_hash=frozen_hash, same_frozen_base=True, matches_corresponding_baseline=True, seed=seed))
    ...
    monitor_training(model,'train_monitor_epoch0')
    step=0;start=time.time()
    for epoch in range(1,51):
        order=d.np.random.default_rng(seed+epoch).permutation(len(ds)).tolist()
        ...
        for j,batch in enumerate(loader):
            ...
            if j==0 or step%20==0:
                status=dict(stage='training',arm='A0_e50_no_clip_'+arm,epoch=epoch,epochs=50,epoch_step=j+1,epoch_size=expected_count,
        ...
        train_fit=monitor_training(model,f'train_monitor_epoch{epoch}')
        if val['dice']>best['dice']:
            best=val;best_epoch=epoch;d.checkpoint(model,D/'best.pt');d.save(D/'best.json',dict(epoch=epoch,**best))
        d.append(D/'epochs.jsonl',dict(epoch=epoch,step=step,validation=val,best_epoch=best_epoch,train_label_fit=train_fit,
            loss_components_mean={k:float(d.np.mean([x['loss_components'][k] for x in updates])) for k in updates[0]['loss_components']},
    ...
    assert d.tensor_hash((n,p) for n,p in model.named_parameters() if not p.requires_grad)==frozen_hash
    d.save(D/'WEIGHTS_FROZEN.json',dict(best_epoch=best_epoch,validation=best,total_steps=step,best_sha256=d.sha(D/'best.pt'),final_epoch=50,final_sha256=d.sha(D/'epoch50.pt')))
    ...
    d.save(D/'results.json',dict(arm='A0_e50_no_clip_'+arm,best_epoch=best_epoch,validation=best,test=result,test_empty=empty_result,training_pool_size=expected_count,
        primary_prompt='colon polyp',empty_text_role='predeclared diagnostic, not test-based prompt selection'))
    d.load_adapter(model,d.torch.load(D/'epoch50.pt',map_location='cpu',weights_only=True))
    d.save(D/'status.json',dict(stage='test_final',epoch=50,best_epoch=best_epoch))
    final_result=d.evaluate(model,'test','test_final')
    d.TEXT_PROMPT=''
    final_empty=d.evaluate(model,'test','test_final_empty')
    d.TEXT_PROMPT='colon polyp'
    final_val=json.loads((D/'validation_epoch50/summary.json').read_text())
    d.save(D/'final_results.json',dict(epoch=50,seed=seed,validation=final_val,test=final_result,test_empty=final_empty,checkpoint_sha256=d.sha(D/'epoch50.pt')))
    report='\n'.join([f'# {arm}新设置训练结果','',f'Best epoch: {best_epoch}; Val Dice: {best["dice"]:.9f}',
        ...
        '原图直接1008输入；256逐图宏平均指标；本次单种子的权重按有文本validation冻结后才执行test。空文本为预先约定的补充消融。',''])
    report += f'\nFinal epoch50 Val Dice: {final_val["dice"]:.9f}; Test Dice: {final_result["dice"]:.9f}; Empty Test Dice: {final_empty["dice"]:.9f}\n'
