...
def summarize(name):
 ...
 if not (d/'PREDICTIONS_FROZEN.json').exists():save(d/'PREDICTIONS_FROZEN.json',dict(time=time.time(),router=False,prediction_hashes={v:[(r['route_id'],r['forward_mask_sha256']) for r in rows] for v,rows in sets.items()}))
 ...
 for variant,rows in sets.items():
  ...
  fixed={str(b):dict(n=s['ntest'],dice=float(np.mean([r['dice'] for r in rows if r['bridge_count']==b])),iou=float(np.mean([r['iou'] for r in rows if r['bridge_count']==b])),q_cycle=(float(np.mean([r['q_cycle'] for r in rows if r['bridge_count']==b and r['q_cycle'] is not None])) if any(r['q_cycle'] is not None for r in rows if r['bridge_count']==b) else None),q_cycle_n=sum(r['q_cycle'] is not None for r in rows if r['bridge_count']==b)) for b in range(7)}
