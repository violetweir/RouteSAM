"""A2 seed2026 ablation: original1008, low LR cosine, NO gradient clipping."""
...
def main():
    ...
    d.save(D/'status.json', dict(stage='initializing', arm='A2_low_lr_no_clip_'+arm, gpu=d.args.gpu))
    ...
    scheduler = d.torch.optim.lr_scheduler.CosineAnnealingLR(trainer.optimizer, T_max=total_steps, eta_min=1e-7)
    if d.args.smoke:
        sample = ds[0]
    ...
    for epoch in range(1,11):
        ...
        for j,batch in enumerate(loader):
            ...
            expected_lr=1e-7+(1e-5-1e-7)*(1+math.cos(math.pi*(step-1)/total_steps))/2
            assert math.isclose(info['learning_rate'],expected_lr,rel_tol=1e-9)
            d.append(D/'updates.jsonl',dict(epoch=epoch,step=step,**info))
            if j==0 or step%20==0:
                status=dict(stage='training',arm='A2_low_lr_no_clip_'+arm,epoch=epoch,epochs=10,epoch_step=j+1,epoch_size=expected_count,
    ...
    d.save(D/'results.json',dict(arm='A2_low_lr_no_clip_'+arm,best_epoch=best_epoch,validation=best,test=result,test_empty=empty_result,training_pool_size=expected_count,
