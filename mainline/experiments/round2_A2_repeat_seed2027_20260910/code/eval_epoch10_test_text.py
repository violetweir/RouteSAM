...
d.D=d.R/'audits/epoch10_test_text'
def main():
 d.D.mkdir(parents=True,exist_ok=False)
 src=d.R/'runs/A2/epoch10.pt';dst=d.D/'epoch10.pt';shutil.copy2(src,dst);digest=d.sha(dst);assert digest==d.sha(src)
 val=json.loads((d.R/'runs/A2/validation_epoch10/summary.json').read_text())
 d.save(d.D/'FROZEN.json',dict(epoch=10,seed=2027,sha256=digest,validation=val,prompt='colon polyp',input='original1008',metric=256,reason='user requested epoch10 diagnostic'))
 ...
 d.save(d.D/'results.json',dict(epoch=10,seed=2027,validation=val,test=result,checkpoint_sha256=digest))
