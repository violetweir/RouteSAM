...
d.D=d.R/'audits/epoch6_test_text'
def main():
 d.D.mkdir(parents=True,exist_ok=False)
 src=d.R/'runs/A2/epoch6.pt';dst=d.D/'epoch6.pt';shutil.copy2(src,dst);digest=d.sha(dst);assert digest==d.sha(src)
 val=json.loads((d.R/'runs/A2/validation_epoch6/summary.json').read_text())
 d.save(d.D/'FROZEN.json',dict(epoch=6,seed=2027,sha256=digest,validation=val,prompt='colon polyp',input='original1008',metric=256,reason='user requested current validation best during training'))
 ...
 d.save(d.D/'results.json',dict(epoch=6,seed=2027,validation=val,test=result,checkpoint_sha256=digest))
