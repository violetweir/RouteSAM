...
class Trainer(nn.Module):
    ...
    def test(self, val_loader):
        if not self.args.unet_checkpoint or not self.args.trained_sam_checkpoint:
            raise ValueError("test mode requires --unet_checkpoint and --trained_sam_checkpoint")
        ...
        self.sam_model.load_state_dict(
            torch.load(self.args.trained_sam_checkpoint, map_location=self.args.device), strict=True
        )
        self.sam_model.eval()
        self.Unet.eval()
        ...
        for i_batch, sampled_batch in enumerate(val_loader):
            ...
            metrics_unet = eval(val_label, pred_UNet_soft)
            avg_dice_unet += metrics_unet[0]
            avg_iou_unet += metrics_unet[1]
        ...
        avg_dice_unet = avg_dice_unet / len(val_loader)
        avg_iou_unet = avg_iou_unet / len(val_loader)
        ...
        print('  unet_mean_dice : %f ' % (avg_dice_unet))
        print('  unet_mean_iou : %f ' % (avg_iou_unet))
