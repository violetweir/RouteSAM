...
def parse_args():
    parser = argparse.ArgumentParser(description="SC-SAM semi-supervised CVC-ClinicDB training")
    ...
    parser.add_argument("--encoder_adapter", action=argparse.BooleanOptionalAction, default=True)
...
def save_protocol(args, train_dataset, output_dir):
    labeled = [str(pair[0]) for pair in train_dataset.pairs[: args.labeled_num]]
    unlabeled = [str(pair[0]) for pair in train_dataset.pairs[args.labeled_num :]]
    ...
    Path(output_dir, "protocol.json").write_text(json.dumps(protocol, indent=2), encoding="utf-8")
...
def train(args):
    ...
    if args.labeled_num is None:
        args.labeled_num = max(1, int(round(len(train_dataset) * args.labeled_ratio)))
    ...
    save_protocol(args, train_dataset, output_dir)
    logging.info("SC-SAM ClinicDB protocol: %d labeled + %d unlabeled", args.labeled_num, len(train_dataset) - args.labeled_num)
