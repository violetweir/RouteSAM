...
def eval(y_true, y_pred, thr=0.5, epsilon=0.001):
    ...
    if y_pred.sum() == 0 and y_true.sum() == 0:
        single_class_res.append(0)
        single_class_res.append(0)
        return single_class_res
    if y_pred.sum() == 0 or y_true.sum() == 0:
        single_class_res.append(100)
        single_class_res.append(100)
        return single_class_res
    single_class_res.append(metric.binary.hd95(y_pred, y_true))
    single_class_res.append(metric.binary.asd(y_pred, y_true))
    return single_class_res
