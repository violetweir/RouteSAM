    ...
    for position, route in enumerate(pending, 1):
        ...
        row = {
            **route,
            ...
            # Dice-only evaluation: the target-to-anchor return pass is omitted
            # because it cannot change the already-produced forward mask.
            "q_cycle": None,
            "cycle_success": None,
            "cycle_failure_reason": "not_run_dice_only",
            "cycle_candidate_count": None,
            "cycle_sam_score": None,
        ...
        print(
            ...
            f"dice={gt_metric['dice']:.4f}" if use_target_gt
            else f"[{position}/{len(pending)}] {route['target_id']} {route['route_type']}",
            flush=True,
        )
...
def main() -> None:
    ...
    for bridge_count in range(8):
        ...
        bridge_rows = [row for row in rows if int(row["bridge_count"]) == bridge_count]
        cycles = [row.get("q_cycle") for row in bridge_rows if row.get("q_cycle") is not None]
        values = [value for value in values if value is not None]
        if bridge_rows:
            summary[f"bridge_{bridge_count}"] = {
                "n": len(bridge_rows),
                **({"dice": float(np.mean(values))} if values else {}),
                **({"q_cycle": float(np.mean(cycles))} if len(cycles) == len(bridge_rows) else {}),
            }
