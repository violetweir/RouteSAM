...
TRAIN_DIR=/Data_8TB/lht/DG-GroupUNet/experiments/wacv2027/T11_sam3_lowlabel_ft_kvasir_budgets/finetune_1pct_plus_hq_pseudo_finite_guard_lr025_eval1_20260806_seed2026
FINAL_CKPT="$TRAIN_DIR/checkpoints/checkpoint_1.pt"
NAME=ft_1pct_plus_hq_pseudo_finite_guard_lr025_eval1_best_saved_ckpt1_20260806
...
TRAIN_PATTERN=kvasir_1pct_plus_hq_pseudo_finite_guard_lr025_eval1_20260806_seed2026.yaml
...
while [[ ! -f "$FINAL_CKPT" ]]; do
  if ! pgrep -u violet -f "$TRAIN_PATTERN" >/dev/null; then
    echo "[watch] training ended before checkpoint_1.pt; aborting"
    exit 2
  fi
  date
  sleep 120
