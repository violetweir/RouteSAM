...
def main() -> None:
    ...
    parser.add_argument("--max-bridge", type=int, default=7)
    ...
    routes = sorted(
        (
            row
            for row in read_jsonl(mode_root / f"{args.split}_pool0_stage1/routes.jsonl")
    ...
    if not routes:
        raise SystemExit(
            f"No routes in bridge range [{args.min_bridge}, {args.max_bridge}]"
        )
