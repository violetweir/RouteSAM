#!/usr/bin/env python3
"""Build the 1% + first-expansion-pool COCO dataset: 8 GT anchors + 491
mainline pseudo labels (phase1/pseudo_manifest_original.jsonl).

Mirrors build_phase1_round2_dataset.py but for the original (first-round) pool,
and tolerates manifests that do not carry q_model_* fields.
"""
...
def main() -> None:
    ...
    parser.add_argument("--source", type=str, default="pseudo491_mainline")
        ...
        annotations.append(
            {
                ...
                "iscrowd": 0,
                "source_type": args.source,
                "q_multi": float(row.get("q_multi", 0.0)),
                "q_return": float(row.get("q_return", 0.0)),
                "q_model_mean": float(row.get("q_model_mean", 0.0)),
                "q_model_min": float(row.get("q_model_min", 0.0)),
    ...
    for split in ("val", "test"):
        ...
        if not (out_split / "images").exists():
            try:
                ...
                shutil.copytree(
                    args.base_dataset / split / "images",
                    out_split / "images",
                    dirs_exist_ok=True,
                )
        shutil.copy2(
