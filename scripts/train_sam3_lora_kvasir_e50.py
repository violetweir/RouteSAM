...
Validation Strategy:
  - During training: compute validation loss and direct semantic Dice every epoch
  - After training: run downstream route/B7 validation for checkpoint selection

Direct Dice is computed from the same validation forward pass, so it adds no
extra model inference. Queries with class probability >= 0.5 are unioned into
one foreground mask after mask-probability thresholding at 0.5.
...
# Load the MedSAM3 LoRA implementation from the repo (do not require MedSAM3's
# own sam3 package on sys.path).
import importlib.util
_LORA_LAYERS_PATH = (
    Path(__file__).resolve().parent.parent / "MedSAM3" / "lora_layers.py"
)
_lora_spec = importlib.util.spec_from_file_location("lora_layers", _LORA_LAYERS_PATH)
if _lora_spec is None or _lora_spec.loader is None:
    raise RuntimeError(f"Cannot import {_LORA_LAYERS_PATH}")
lora_layers = importlib.util.module_from_spec(_lora_spec)
sys.modules["lora_layers"] = lora_layers
_lora_spec.loader.exec_module(lora_layers)
LoRAConfig = lora_layers.LoRAConfig
apply_lora_to_model = lora_layers.apply_lora_to_model
save_lora_weights = lora_layers.save_lora_weights
load_lora_weights = lora_layers.load_lora_weights
count_parameters = lora_layers.count_parameters
...
# Full instance metrics (mAP, cgF1, NMS) remain in validate_sam3_lora.py.
...
class COCOSegmentDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not img_path.exists():
            img_path = self.split_dir / "images" / img_info['file_name']
...
class SAM3TrainerNative:
    def __init__(self, config_path, multi_gpu=False, resume_lora=None, start_epoch=0):
        with open(config_path, "r") as f:
            self.config = yaml.safe_load(f)
        self.start_epoch = start_epoch
        self.seed = int(self.config["training"].get("seed", 42))
        ...
        self.model = build_sam3_image_model(
            ...
            compile=False,
            checkpoint_path=self.config["model"].get(
                "checkpoint_path",
        ...
        if resume_lora is not None:
            print_rank0(f"Resuming LoRA weights from {resume_lora}")
            load_lora_weights(self.model, resume_lora)
    ...
    def train(self):
        ...
        if self.start_epoch > 0:
            stats_path = out_dir / "val_stats.json"
            if stats_path.exists():
                ...
                previous = [
                    ...
                    if int(row.get("epoch", 0)) <= self.start_epoch
                ]
                if previous:
                    best_val_loss = min(float(row["val_loss"]) for row in previous)
                    best_val_dice = max(
                        float(row.get("val_dice", -1.0)) for row in previous
                    )
                    print_rank0(
                        f"Restored best validation loss {best_val_loss:.6f} "
                        f"and best direct Dice {best_val_dice:.6f} "
                        f"from epochs 1-{self.start_epoch}"
                    )
        ...
        for epoch in range(self.start_epoch, epochs):
            ...
            # Save the completed training epoch before validation.  Validation
            # uses SAM3's inference-only BF16 kernels and may fail independently
            # of training; a validation failure must not discard a full epoch.
            if has_validation and val_loader is not None and is_main_process():
                ...
                save_lora_weights(
                    model_to_save, str(out_dir / f"epoch_{epoch+1}_lora_weights.pt")
                )

            # Validation loss plus ordinary direct semantic Dice.  Dice reuses
            # the same forward pass and therefore adds only mask postprocessing.
            if has_validation and val_loader is not None:
                ...
                with torch.no_grad():
                    ...
                    for batch_dict in val_pbar:
                        ...
                        # SAM3's no-grad ViT MLP uses fused BF16 inference
                        # kernels.  LoRA base weights remain FP32, so autocast
                        # is required to keep Linear/LoRA matmuls dtype-safe.
                        with torch.autocast(
                        ...
                        if final_outputs is None:
                            raise RuntimeError("SAM3 validation produced no final output")
                        logits = final_outputs["pred_logits"].float().squeeze(-1)
                        mask_probs = torch.sigmoid(final_outputs["pred_masks"].float())
                        if logits.shape[0] != 1:
                            raise RuntimeError(
                                "Direct validation Dice currently requires batch_size=1"
                            )
                        ...
                        gt = find_targets[-1]["masks"].bool().any(dim=0)
                        ...
                        denominator = prediction.sum(dtype=torch.float64) + gt.sum(
                            dtype=torch.float64
                        )
                        direct_dice = (
                        ...
                        val_dices.append(float(direct_dice))
                ...
                if self.multi_gpu:
                    val_tensor = torch.tensor(
                        [sum(val_losses), len(val_losses), sum(val_dices), len(val_dices)],
                        device=self.device,
                        dtype=torch.float64,
                    )
                    dist.all_reduce(val_tensor, op=dist.ReduceOp.SUM)
                    avg_val_loss = float(val_tensor[0] / val_tensor[1])
                    avg_val_dice = float(val_tensor[2] / val_tensor[3])

                print_rank0(
                    f"\nEpoch {epoch+1}/{epochs} - Train Loss: {avg_train_loss:.6f}, "
                    f"Val Loss: {avg_val_loss:.6f}, Direct Val Dice: {avg_val_dice:.6f}"
                )
                ...
                if is_main_process():
                    ...
                    if avg_val_dice > best_val_dice:
                        ...
                        save_lora_weights(
                            model_to_save,
                            str(out_dir / "best_val_dice_lora_weights.pt"),
                        )
                        print(
                            f"✓ New direct-Dice best saved "
...
Examples:
  ...
  Multi-GPU (all 4 GPUs):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a saved LoRA weights file to resume from (epoch_N_lora_weights.pt)",
    )
    parser.add_argument(
        "--start-epoch",
        type=int,
        default=0,
        help="Epoch index to start training from when resuming (0-based, epoch numbering in filenames is 1-based)",
    )
