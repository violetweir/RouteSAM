...
class Attention(nn.Module):
    ...
    def _apply_rope(self, q, k) -> Tuple[Tensor, Tensor]:
        ...
        if self.freqs_cis.shape[0] == q.shape[-2]:
            ...
            # Input token grid differs from the precomputed size (e.g. a non-default
            # input resolution): recompute RoPE frequencies for the actual grid.
            h = w = int(math.sqrt(q.shape[-2]))
            scale_pos = 1.0
