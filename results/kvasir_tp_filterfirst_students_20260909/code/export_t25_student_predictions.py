#!/usr/bin/env python3
"""Export frozen T24 student predictions for T25 without changing weights."""
...
def main() -> None:
    ...
    for split in args.splits:
        ...
        with torch.inference_mode():
            for row in sorted(metadata, key=lambda item: item["merged_id"]):
                ...
                tensor = normalize(torch.from_numpy(array).permute(2, 0, 1))
                _, probabilities = model(tensor.unsqueeze(0).cuda())
                ...
                Image.fromarray(np.rint(foreground * 65535).astype(np.uint16)).save(
                    probability_path
                )
    ...
    (args.output_root / "EXPORT_COMPLETE").write_text("complete\n")
