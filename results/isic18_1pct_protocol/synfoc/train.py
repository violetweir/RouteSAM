...
def train(args, snapshot_path):
    ...
    if args.dataset == 'fundus':
        ...
        domain_len = [len(list(Path(train_data_path, 'train', 'images').glob('*.png')))]
    ...
    if args.dataset == 'ClinicDB':
        ...
        all_names = sorted(p.name for p in Path(train_data_path, 'train', 'images').glob('*.png'))
        name_to_idx = {name: idx for idx, name in enumerate(all_names)}
        labeled_names = [Path(line.strip()).name for line in Path(args.labeled_list).read_text().splitlines() if line.strip()]
    ...
    if args.dataset == 'ClinicDB':
        ...
        protocol = {
            ...
            'split': {'train': data_num, 'validation': len(validation_dataset),
                      'test': len(list(Path(train_data_path, 'test', 'images').glob('*.png')))},
    ...
    if args.dataset == 'ClinicDB' and args.save_model:
        final_unet_checkpoint = os.path.join(snapshot_path, 'unet_final_model.pth')
        final_sam_checkpoint = os.path.join(snapshot_path, 'SAM_final_model.pth')
        torch.save(unet_model.state_dict(), final_unet_checkpoint)
        try:
            SAM_model.save_lora_parameters(final_sam_checkpoint)
        except AttributeError:
            SAM_model.module.save_lora_parameters(final_sam_checkpoint)
        logging.info('save final unet model to {}'.format(final_unet_checkpoint))
        logging.info('save final SAM model to {}'.format(final_sam_checkpoint))
        ...
        logging.info('final one-shot test: final checkpoints')
        final_ckpt_unet_dice = test(args, unet_model, final_test_loader, max_epoch,
                                    writer, model_name='unet')
        final_ckpt_sam_dice = test(args, SAM_model, final_test_loader, max_epoch,
        ...
        best_ckpt_unet_dice = test(args, unet_model, final_test_loader, max_epoch,
                                   writer, model_name='unet')
        best_ckpt_sam_dice = test(args, SAM_model, final_test_loader, max_epoch,
                                  writer, model_name='SAM')
        summary = {
            'best_validation': {
            ...
            'test': {
                ...
                'unet_dice': float(sum(best_ckpt_unet_dice) / len(best_ckpt_unet_dice)),
                'sam_dice': float(sum(best_ckpt_sam_dice) / len(best_ckpt_sam_dice)),
            },
            'test_best_validation': {
                'images': len(final_test_dataset),
                'unet_dice': float(sum(best_ckpt_unet_dice) / len(best_ckpt_unet_dice)),
                'sam_dice': float(sum(best_ckpt_sam_dice) / len(best_ckpt_sam_dice)),
            },
            'test_final': {
                'images': len(final_test_dataset),
                'unet_dice': float(sum(final_ckpt_unet_dice) / len(final_ckpt_unet_dice)),
                'sam_dice': float(sum(final_ckpt_sam_dice) / len(final_ckpt_sam_dice)),
            },
        }
