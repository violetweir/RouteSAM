...
def fast_routes(stage, records, support, patch_mean, cond_scores, split, beam_width=32, transition_sim=None):
    ...
    sim = (patch_mean @ patch_mean.T).astype(np.float64) if transition_sim is None else np.asarray(transition_sim, dtype=np.float64)
