...
import os
os.environ['CUDA_VISIBLE_DEVICES']='';os.environ['OPENBLAS_NUM_THREADS']='4';os.environ['OMP_NUM_THREADS']='4'
...
P=Path('/Data_8TB/lht/PseudoVideo-SAM3-X3-B7');R=P/'work/kvasir_pc_prompt_refine_20260908';TOK=P/'work/kvasir_pc_evidence_20260908/tokens'
...
_original_position_builder=builder._create_position_encoding
builder._create_position_encoding=lambda precompute_resolution=None:_original_position_builder(precompute_resolution=None)
_original_get_coords=TransformerDecoder._get_coords
TransformerDecoder._get_coords=staticmethod(lambda H,W,device:_original_get_coords(H,W,'cpu'))
def cpu_pin_memory(self,*args,**kwargs):
    assert self.device.type=='cpu'
    return self
torch.Tensor.pin_memory=cpu_pin_memory

ap=argparse.ArgumentParser();ap.add_argument('--limit',type=int,default=100);args=ap.parse_args()
torch.set_num_threads(4);torch.set_num_interop_threads(1);R.mkdir(parents=True,exist_ok=True)
config={'split':'validation only','student':False,'fixed_inputs':'Original five-fold OOF TP anchor, path and default mask; original 8 train supports',
 ...
 'decoder':'SAM3-base image geometric box grounding, resolution256; implicit visual token, no polyp text; highest model confidence query; empty result falls back to TP',
 ...
 'precision':'CPUfloat32; isolated fused MLP replacement, same as previous evidence extraction; no shared source modifications',
...
if pending:
    t0=time.time();print('building CPU image model; pending',len(pending),flush=True)
    model=build_sam3_image_model(device='cpu',checkpoint_path=CKPT,load_from_HF=False,enable_inst_interactivity=False,compile=False).eval()
    for blk in model.backbone.vision_backbone.trunk.blocks:
        attn=blk.attn
        if attn.use_rope and attn.freqs_cis is not None and blk.window_size==0:
            attn.register_buffer('freqs_cis',attn.compute_cis(end_x=18,end_y=18,scale_pos=attn.rope_pt_size[0]/18 if attn.rope_interp else 1.).cpu())
    processor=Sam3Processor(model,resolution=256,device='cpu',confidence_threshold=0.)
    with torch.inference_mode():
        visual=model.backbone.forward_text(['visual'],device='cpu')
        ...
        for ni,r in enumerate(pending):
            ...
            print('refined',len(done)+ni+1,'/',args.limit,'elapsed',round(time.time()-t0,1),flush=True)
