...
summary={'masks_verified':300,'guarded_masks_exported':300,'mask_dice_recomputed':True,'box_coordinates_verified':True,'test_evaluated':False,
 ...
 'cpu_compatibility':['float32 Linear+GELU replaces forced-bfloat16 fused MLP','skip CUDA-only positional-cache precomputation; compute on demand on CPU','decoder coordinate-cache allocation on CPU','pin_memory no-op for CPU-only process'],
 'source_files_unmodified':'Compatibility changes are Python process-local bindings; no shared SAM3 source edits.',
