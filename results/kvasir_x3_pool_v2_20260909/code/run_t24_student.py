...
PROJECT_ROOT = Path('/Data_8TB/lht/PseudoVideo-SAM3-X3-B7')
SCSAM_ROOT = Path(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--experiment", choices=("S0", "S1", "S2", "S3", "X3"), required=True
    )
    ...
    parser.add_argument("--quality-weight-mode", choices=("normalized_q_multi", "manifest"), default="normalized_q_multi")
...
class T22StudentDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        mask = load_probability(row["target"])
        pixel_weight_path = row.get("pixel_weight_path")
        pixel_weight = (
            load_probability(pixel_weight_path)
...
def main() -> None:
    ...
    test_set = None if args.defer_test else T22StudentDataset(args, "test", transforms["valid_test"])
    ...
    protocol.update(
        ...
        checkpoint_rule=(
            ...
            "validation Dice improves; test is deferred to the frozen final pipeline when defer_test is set"
        ),
        entropy_weight=0.0,
        ...
        pseudo_weight_mode=(
            ...
            if args.experiment == "S0"
                ...
                else args.quality_weight_mode
            )
        ),
        quality_normalization=("absolute manifest weights; no minmax rescaling" if args.quality_weight_mode == "manifest" else "fixed over accepted pseudo labels; clip to [0.2,1.0]"),
    )
    ...
    test_loader = None if args.defer_test else DataLoader(
    ...
    try:
        while iteration < args.max_iterations:
            try:
                ...
                train_iter = iter(loader)
                batch = next(train_iter)
            expected_labeled = len(batch["is_labeled"]) if args.experiment == "S0" else args.labeled_bs
    ...
    summary = {
        ...
        "test": None if args.defer_test else evaluate(model, test_loader),
